...
def test_create_message(test_db):
    ...
    message_data = message_schemas.MessageCreate(content="Test message", sender="TestPlayer1")
...
def test_get_chat(test_db):
    ...
    game_data = game_schemas.GameCreate(name="Test Game 2", min_players=4, max_players=6)
    created_game = game_crud.create_game(game_data)

    message_data = message_schemas.MessageCreate(content="Test message 1", sender="TestPlayer1")
    created_message = message_crud.create_message(message_data, created_game.id)
    message_data = message_schemas.MessageCreate(content="Test message 2", sender="TestPlayer2")
    created_message = message_crud.create_message(message_data, created_game.id)
    message_data = message_schemas.MessageCreate(content="Test message 3", sender="TestPlayer3")
    created_message = message_crud.create_message(message_data, created_game.id)
...
def test_chat_endpoints(test_db):
    ...
    # create a game
    game_data = game_schemas.GameCreate(name="Test Game 3", min_players=4, max_players=6)
    created_game = game_crud.create_game(game_data)

    response = test_app.put(f"/game/{created_game.id}/send-message",
    ...
    test_app.put(f"/game/{created_game.id}/send-message",
